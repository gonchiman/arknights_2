...
export interface OperatorDetailCommonProps {
...
export interface OperatorDetailContentProps extends OperatorDetailCommonProps {
  variant: 'modal' | 'page'
  titleId: string
  headerAction: ReactNode
}

export interface OperatorDetailContentHandle {
  dismissClassifier: () => boolean
  focusTitle: () => void
}
...
export const OperatorDetailContent = forwardRef<OperatorDetailContentHandle, OperatorDetailContentProps>(
  function OperatorDetailContent({
    operator,
    comparisonOperators,
    skills,
    overrides,
    ...
    useImperativeHandle(ref, () => ({
      dismissClassifier: () => {
        if (!selectedClassifierSkill) return false
    ...
    return (
      <>
        <article
          className={variant === 'modal' ? 'operator-detail-modal' : 'operator-detail-page-panel'}
          inert={selectedClassifierSkill ? true : undefined}
        >
          <header className="operator-detail-modal-header">
            <div>
              <span>OPERATOR DETAIL</span>
              <Heading ref={titleRef} id={titleId} tabIndex={-1}>{operator.name}</Heading>
